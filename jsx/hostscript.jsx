// ============================================================
//  SpeedFlow Pro – hostscript.jsx  (ExtendScript / AE side)
// ============================================================

// JSON Polyfill for older/strict ExtendScript environments
...
function createNull(isNull) {
    ...
    if (makeNull) {
        ...
        // Force full duration immediately after creation
        try { newLayer.startTime = 0; newLayer.inPoint = 0; newLayer.outPoint = comp.duration; } catch(e){}
    }
...
function reverseLayerOrder() {
    ...
    // Collect indices sorted ascending
    var indices = [];
...
function applyFlow(opts) {
    ...
    // AE Influence = percentage of segment duration the handle covers
    // Out influence = cp1x * 100 (handle from start going forward)
    // In  influence = (1 - cp2x) * 100 (handle from end going backward)
...
// Get the magnitude of value change per second between two keyframes
// Returns a single number for OneD/Spatial, or an array of numbers for TwoD/ThreeD
...
function applyEaseToProp(prop, isLinear, outSlope, outInf, inSlope, inInf, kfMode) {
    ...
    // Number of KeyframeEase objects needed: 1 for Spatial/OneD, 2/3/4 for Multi-dimensional
    var dimCount = 1;
    ...
    for (var idx = 0; idx < keysToApply.length; idx++) {
        ...
        try {
            ...
            for (var d = 0; d < dimCount; d++) {
                // Get the linear speed for this specific dimension
...
function readFlowFromSelected() {
    ...
    try {
        ...
        // keyOutTemporalEase(k) = the ease leaving keyframe k (OUT handle)
        // keyInTemporalEase(k)  = the ease arriving at keyframe k (IN handle)
        var easeOut = prop.keyOutTemporalEase(firstKey); // leaving first key
        var easeIn  = prop.keyInTemporalEase(lastKey);   // arriving at last key
        ...
        // Compute linear segment speed to normalize the slope
...
// ══════════════════════════════════════════════════════════
//  TARIK TOOLS API — HTTP Fonksiyonları
//  Not: CEP ExtendScript'te Socket ile HTTP yapılır
// ══════════════════════════════════════════════════════════
...
function httpPost(url, apiKey, bodyStr) {
    ...
    var conn = new Socket();
    conn.encoding = 'UTF-8';
    conn.timeout = 60; // 60 saniye timeout
    ...
    // Chunked transfer encoding temizle
...
// ── Siteden API Key al (JWT cookie ile) ───────────────────
...
function importSubtitleToAE(segmentsJson, timeOffset) {
    ...
    // Text layer oluştur (üstten alta sıra ile)
    for (var i = segments.length - 1; i >= 0; i--) {
        try {
            var seg = segments[i];
            var textVal = String(seg.text || "");
            if (!textVal.trim()) continue; // Boş metinleri atla
...
// ── DOSYADAN AE'YE AKTAR (GÜVENLİ YÖNTEM) ─────────────────
...
function exportSelectedLayerAudio() {
    try {
        ...
        // Diğer render kuyruğu öğelerini devre dışı bırak
        ...
        // Solo ve Work Area'yı geçici olarak değiştir
        var oldSoloStates = [];
        var oldAudioStates = [];
        for (var i = 1; i <= comp.numLayers; i++) {
        ...
        comp.workAreaDuration = comp.frameDuration; // shrink first to avoid bounds error
        comp.workAreaStart = newStart;
        comp.workAreaDuration = newDuration;

        // Temp klasöründe benzersiz bir wav dosyası oluştur
        var tempDir = Folder.temp;
        var wavFile = new File(tempDir.fsName + "/ae_temp_audio_" + new Date().getTime() + ".wav");
        ...
        // Apply audio-only MP3/WAV template
        ...
        // Video çıktısını kapatıp sadece ses çıktısını aktifleştirerek render hızını maksimize et
        try {
        ...
        rqItem.render = true; // Sadece bu öğe aktif olarak render edilecek
        ...
        // Render bittikten sonra odağı hemen kompozisyon izleyicisine (Viewer) geri al
        try {
            if (app.activeViewer) {
        ...
        // Dosya uzantısı kontrol ve fallback araması
        if (!wavFile.exists) {
...
function importIsolatedAudioToAE(filePath, startTimeVal, inPointVal, outPointVal, muteOriginal, stemName) {
    ...
    try {
        ...
        // Orijinal seçili katmanları kaydet (yeni katman eklenmeden önce)
        ...
        if (muteOriginal) {
            for (var i = 0; i < originalSelected.length; i++) {
                ...
                originalSelected[i].selected = false; // Seçimi bırak
            }
        }
        
        newLayer.selected = true; // Sadece yeni katman seçili kalsın
...
// =========================================================================
// TARIK SPEED PRO EXACT LOGIC PORTS (From Original Script)
// =========================================================================
...
function createNull(shiftPressed) {
    ...
    // Sort selected layers by index ascending so layers[0] is the topmost
    var layers = [];
    for (var i = 0; i < comp.selectedLayers.length; i++) {
    ...
    // Respect selected layer size & position
    var w = comp.width;
    var h = comp.height;
    ...
    if (shiftPressed === "true" || shiftPressed === true) {
        ...
        // label atanmiyor, AE'nin default rengi kalsın
        if (layerPosition !== null) {
            try {
                if (isThreeD) {
...
function createAdjustment(shiftPressed) {
    // Both handled in createNull for simplicity since they use the same UI button in TarikSpeed
...
function processPreComp(comp, selectedLayers, preCompName, originalFPS) {
    ...
    preComp.frameRate = 60; // default preCompFPS
...
function doPrecomp(shiftPressed) {
    ...
    try {
        ...
        comp.frameRate = 60; // settings.preCompFPS
